...
function transformDictionary(dictionary) {
  ...
  const iteratorEntries = dictionary.entries();

  for (const [key, value] of dictionary) {
    keys.push(iteratorKeys.next().value);
    values.push(iteratorValues.next().value);
    entries.push(iteratorEntries.next().value);
  }
...
function getFilteredDictionarySize(dictionary, filter) {
  ...
  for (const [key, value] of dictionary) {
    if (filter(key, value)) { 
    filteredDictionary.set(iteratorEntries.next().value);
  }
   };
...
function sortByValues(dictionary) {
  ...
  const arrIter = [...dictionary];
 
  arrIter.sort((elem1, elem2) => {
    return elem1 < elem2 ? 1 : -1;})
  
  dictionary = new Map(arrIter);
...
function resetDictionary(dictionary, maxSize) {
  ...
  const sizeDictionary = dictionary.size;

  if (sizeDictionary > maxSize) {
    dictionary.clear();
    return true;
  } else {return false};
}
...
function convertSetToDictionary(set) {
  ...
  set.forEach((value, index, array) => {
    letterFirst = value.toString().slice(0,1);
    simbolFirst = letterFirst.charCodeAt();
    map.set(value, simbolFirst);
  });
